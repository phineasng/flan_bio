...
def init_trainer(config, train_loader, val_loader, test_loader, best_model=False):
    ...
    if hasattr(config, 'model_class') and config.model_class == 'DiSENN':
        ...
        trainer = SENN_Trainer(config, train_loader, val_loader, test_loader)
    return trainer


class SENN_Trainer:
    def __init__(self, config, train_loader, val_loader, test_loader, save_ckpt=True):
    ...
    def get_metrics(self, validate=True):
        ...
        with torch.no_grad():
            for x, labels in dl:
                ...
                preds.append(torch.exp(y_pred).detach().cpu().numpy())
        ...
        return roc_auc_score(true_labels, preds[:, 1])
